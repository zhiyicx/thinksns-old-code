...
class LogsModel extends Model
{
    ...
    protected $fields = ['id', 'uid', 'uname', 'app_name', 'group', 'action', 'data', 'ctime', 'url', 'isAdmin', 'ip', 'keyword'];
    ...
    public function getMenuList($app)
    {
        ...
        if (!file_exists($logsXml)) {
            $this->error = L('PUBLIC_SETTING_FILE', ['file' => $logsXml]);            // 配置文件：{file}不存在
            return false;
        }
    ...
    protected function __paseTemplate($_data)
    {
        ...
        if (!file_exists($logFile)) {
            $this->error = L('PUBLIC_SETTING_FILE', ['file' => $logFile]);            // 配置文件：{file}不存在
            return false;
        }
        ...
        // 异常情况
        $return = ['info' => L('PUBLIC_PERMISSION_POINT_NOEXIST'), 'data' => L('PUBLIC_PERMISSION_POINT_NOEXIST')];            // 权限节点不存在，权限节点不存在
