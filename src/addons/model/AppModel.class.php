...
class AppModel extends Model
{
    ...
    public static $defaultApp = []; // 默认应用字段
    public $_host_type = []; // 应用类型字段
     ...
     * @return array 应用列表数据
     */
    public function getAppList($map = [], $limit = '')
    {
    ...
    public function getAppFrontList($limit = 9)
    {
        ...
        if ($list == false) {
            $list = $this->getAppList(['status' => 1, 'add_front_top' => 1], $limit);
     ...
     */
    public function getAppHash($hashKey = 'app_id', $hashValue = 'app_alias', $map = [])
    {
        $list = $this->getAppList($map);
        $r = [];
        foreach ($list as $v) {
    ...
    public function saveApp($data)
    {
        ...
        if ($data['host_type'] == 0 && !is_dir(APPS_PATH.'/'.$data['app_name'])) {
            return L('PUBLIC_DIRECTORY_NOEXIST', [
                    'dir' => $data['app_name'],
            ]); // {dir}目录不存在
        }
        ...
        if (!empty($data['app_id'])) {
            ...
            // 固定数据内容处理
            empty($oldInfo['child_menu']) && $oldInfo['child_menu'] = [];
