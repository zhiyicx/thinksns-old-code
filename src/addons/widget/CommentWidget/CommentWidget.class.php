...
class CommentWidget extends Widget
{
    ...
    public function render($data)
    {
        ...
        //判断是否有权限评论当前用户
        if ($this->mid != $sourceInfo['uid'] && $data['app_name'] != 'Information') {
    ...
    public function addcomment()
    {
        ...
        if ($this->mid != $uid) {
            ...
            if ($userPrivacy['comment_weibo'] == 1) {
                exit(json_encode(['status' => 0, 'data' => L('PUBLIC_CONCENT_TIPES')]));
            }
        }
        $filterContentStatus = filter_words($data['content']);
        if (!$filterContentStatus['status']) {
            exit(json_encode(['status' => 0, 'data' => $filterContentStatus['data']]));
        }
    ...
    public function parseComment($data, $talkbox)
    {
        ...
        if ($talkbox) {
            ...
            $html .= '<dt class="right">';
            $html .= '<a href="' . U('public/Profile/index', ['uid' => $data['userInfo']['uid']]) . '"><img src="' . $data['userInfo']['avatar_tiny'] . '"></a>';
            $html .= '</dt>';
