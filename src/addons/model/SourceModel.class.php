...
class SourceModel
{
    ...
    public function getSourceInfo($table, $row_id, $_forApi = false, $appname = 'public')
    {
        ...
        switch ($table) {
            ...
            case 'poster':
                ...
                $info['source_url'] = U('poster/Index/posterDetail', [
                        'id' => $row_id,
                ]);
                $info['source_body'] = $poster['title'].'<a class="ico-details" href="'.U('poster/Index/posterDetail', [
                        'id' => $row_id,
                ]).'"></a>';
            ...
            case 'event':
                ...
                $info['source_url'] = U('event/Index/eventDetail', ['id' => $row_id, 'uid' => $event['uid']]);
                $info['source_content'] = ($info['source_user_info'] !== false) ? '发表了一个活动' : '内容已被删除';
                $info['source_body'] = $event['title'].'<a class="ico-details" href="'.U('event/Index/eventDetail', ['id' => $row_id, 'uid' => $event['uid']]).'"></a>';
            ...
            case 'blog':
                ...
                $info['source_url'] = U('blog/Index/show', ['id' => $row_id, 'mid' => $blog['uid']]);
                $info['source_content'] = ($info['source_user_info'] !== false) ? '发表了一篇知识' : '内容已被删除';
                $info['source_body'] = $blog['title'].'<a class="ico-details" href="'.U('blog/Index/show', ['id' => $row_id, 'mid' => $blog['uid']]).'"></a>';
            ...
            case 'photo':
                ...
                $info['source_url'] = U('photo/Index/photo', ['id' => $row_id, 'aid' => $photo['albumId'], 'uid' => $photo['userId']]);
                ...
                $info['album_url'] = U('photo/Index/album', ['uid' => $photo['userId'], 'id' => $photo['albumId']]);
                break;
            case 'vote':
                ...
                $info['source_url'] = U('vote/Index/pollDetail', ['id' => $row_id]);
                $info['source_content'] = ($info['source_user_info'] !== false) ? '发表了一个投票' : '内容已被删除';
                $info['source_body'] = $vote['title'].'<a class="ico-details" href="'.U('vote/Index/pollDetail', ['id' => $row_id]).'"></a>';
            ...
            case 'develop':
                ...
                $info['source_url'] = U('develop/Index/detail', ['id' => $row_id]);
                $info['source_body'] = $develop['title'].'<a class="ico-details" href="'.U('develop/Index/detail', ['id' => $row_id]).'"></a>';
                break;
            case 'weiba_post':
                ...
                $info['source_url'] = U('weiba/Index/postDetail', ['post_id' => $row_id]);
                $info['source_content'] = ($info['source_user_info'] !== false) ? '发表了一个帖子' : '内容已被删除';
                $info['source_body'] = $weiba['title'].'<a class="ico-details" href="'.U('weiba/Index/postDetail', ['post_id' => $row_id]).'"></a>';
                ...
                $info['weiba_url'] = U('weiba/Index/detail', ['weiba_id' => $weiba['weiba_id']]);
                // 获取编辑器中的图片内容
    ...
    private function getInfoFromFeed($table, $row_id, $forApi)
    {
        ...
        $info['source_url'] = U('public/Profile/feed', [
                'feed_id' => $row_id,
                'uid'     => $info['uid'],
        ]);
