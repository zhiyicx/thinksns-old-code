...
class UserModel extends Model
{
     ...
     * @return array 用户列表信息
     */
    public function getUserList($limit = 20, array $map = [], $order = '`uid` DESC')
    {
        // 添加用户表的查询，用于关联查询
        ...
        if (isset($_POST)) {
            ...
            $_POST['uname'] && $map['uname'] = [
                    'LIKE',
                    '%'.t($_POST['uname']).'%',
            ];
            $_POST['email'] && $map['email'] = [
                    'LIKE',
                    '%'.t($_POST['email']).'%',
            ];
     ...
     * @return array 用户列表信息
     */
    public function getList(array $map = [], $limit = 20, $field = '*', $order = '`uid` DESC')
    {
     ...
     * @return array 用户列表信息
     */
    public function getPage(array $map = [], $limit = 20, $field = '*', $order = '`uid` DESC')
    {
    ...
    public function getUserInfo($uid)
    {
        ...
        // # 判断是否有用户缓存
            ...
            $user = $this->_getUserInfo(['uid' => $uid]);
        }
    ...
    public function getUserInfoForSearch($uid, $field = '*')
    {
        ...
        // # 判断是否有用户缓存
            ...
            $this->error = L('PUBLIC_GET_USERINFO_FAIL'); /* # 获取用户信息缓存失败 */
            $user = $this->_getUserInfo(['uid' => $uid], $field);
        }
     ...
     */
    public function getUserInfoByName($userName, array $map = [])
    {
        // if (empty ( $uname )) {
        ...
        $_map = array_merge(['uname' => $userName], $map);
    ...
    private function _getUserInfo(array $map, $field = '*')
    {
        ...
        if (!$user) {
            ...
            $user['avatar_url'] = U('public/Attach/avatar', [
                    'uid' => $user['uid'],
            ]);
            $user['space_url'] = !empty($user['domain']) ? U('public/Profile/index', [
                    'uid' => $user['domain'],
            ]) : U('public/Profile/index', [
                    'uid' => $user['uid'],
            ]);
