...
class ParseTagModel
{
    ...
    public function parseId($content, $cacheOpen = true)
    {
        ...
        foreach ($pluginIds as $key => $value) {
            ...
            if ($cacheOpen && $cacheTime != 0 && C('MEMCACHED_ON')) {
                $cache = service('Cache', ['type' => 'memcache']);
    ...
    private function pregReplaceTag($tagName, $path, $content, $replace = true)
    {
        ...
        if ($replace) {
            $content = preg_replace_callback($preg, [$this, 'parseTagContent'], $content);
        } else {
            preg_replace_callback($preg, [$this, 'parseTagContent'], $content);
        }
