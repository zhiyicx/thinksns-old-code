...
class NewsModel extends Model
{
    ...
    public function getList($limit = 20, $type = 0, $keywords = '', $order = '', $findPage = true)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            //获取评论数量
            $data[$k]['comment_count'] = model('Comment')->where(['app' => 'news', 'table' => 'news', 'is_del' => 0, 'row_id' => $v['news_id']])->count();
        }
        if ($findPage) {
            $list['data'] = $data;
    ...
    public function getSourceInfo($id)
    {
        ...
        if ($find) {
            $info = ['source_user_info' => ''];
            if ($find['uid']) {
                $info['source_user_info'] = model('User')->getUserInfo($find['uid']);
            }
            $info['source_url'] = U('news/Index/detail', ['id' => $id]);
            $info['source_body'] = $find['news_title'].'<a class="ico-details" href="'.U('news/Index/show', ['id' => $id]).'"></a>';
    ...
    {
        return $this->where(['type_id' => $cid])->delete();
    }
}
