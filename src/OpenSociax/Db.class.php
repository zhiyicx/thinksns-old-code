...
class Db extends Think
{
    ...
    protected $beginTime;
    // 数据库表达式
    protected $comparison = ['eq' => '=', 'neq' => '!=', 'gt' => '>', 'egt' => '>=', 'lt' => '<', 'elt' => '<=', 'notlike' => 'NOT LIKE', 'like' => 'LIKE'];
    // 查询表达式
    ...
    protected function parseTable($tables)
    {
        ...
        array_walk($tables, [&$this, 'addSpecialChar']);
                ...
                if (0 === strpos($key, '_')) {
                    ...
                    if (is_array($val)) {
                        if (is_string($val[0])) {
                            ...
                            $count = count($val);
                            if (in_array(strtoupper(trim($val[$count - 1])), ['AND', 'OR', 'XOR'])) {
    ...
    public function selectInsert($fields, $table, $options = [])
    {
        ...
        $sql .= str_replace(
            ['%TABLE%', '%DISTINCT%', '%FIELDS%', '%JOIN%', '%WHERE%', '%GROUP%', '%HAVING%', '%ORDER%', '%LIMIT%'],
            [
     ...
     * @return false | integer
     */
    public function insertAll($datas, $options = [])
    {
        if (!is_array($datas[0])) {
            return false;
        }
