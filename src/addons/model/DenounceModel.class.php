...
class DenounceModel extends Model
{
    ...
    protected $fields = [0 => 'id', 1 => 'from', 2 => 'aid', 3 => 'state', 4 => 'uid', 5 => 'fuid', 6 => 'reason', 7 => 'content', 8 => 'ctime', 9 => 'source_url'];
    ...
    public function deleteDenounce($ids, $state)
    {
        ...
        $weibo_map['feed_id'] = ['IN', $weiboIds];
        $weibo_set = model('Feed')->where($weibo_map)->save(['is_del' => 1]);
        if ($state == 0) {
            $result = $this->where($this->_paramMaps($ids))->save(['state' => '1']);
        } elseif ($state == 1) {
    ...
    public function reviewDenounce($ids)
    {
        $weiboIds = $this->_getWeiboIdsByDenounce($ids);
        $weibo_map['feed_id'] = ['IN', $weiboIds];
        $weibo_set = model('Feed')->where($weibo_map)->save(['is_del' => 0]);
        // 删除举报信息
        $result = $this->where($this->_paramMaps($ids))->delete();
    ...
    public function autoDenounce($id, $uid, $content, $type = 'feed')
    {
        ...
        model('Feed')->where($weibo_map)->save(['is_del' => 1]);
